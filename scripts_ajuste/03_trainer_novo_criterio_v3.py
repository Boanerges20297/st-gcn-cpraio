...
# -*- coding: utf-8 -*-
"""
Treinador Simplificado - Sem Reshape Complexo
"""
...
def main():
    print("\n" + "="*80)
    print("ST-GCN SIMPLIFICADO - Versão 3")
    print("="*80)
    ...
    try:
        # LOAD DATA
        print("\n[1] LOADING DATA...")
        data = torch.load(DATA_DIR / 'tensors' / 'dataset_cvli_novo_criterio.pt')
        if isinstance(data, dict):
            dataset = data.get('data', next(v for v in data.values() if isinstance(v, torch.Tensor)))
        else:
            dataset = data
        
        print(f"    Shape: {dataset.shape}")
        
        with open(DATA_DIR / 'tensors' / 'metadata_cvli.json', 'r', encoding='utf-8') as f:
            meta = json.load(f)
        ...
        # CREATE SEQUENCES - FLATTENED
        print("\n[2] CREATING SEQUENCES...")
        X, y = [], []
        for i in range(len(dataset) - WINDOW):
            X.append(dataset[i:i+WINDOW].mean(dim=0))  # Average over window
            y.append(dataset[i+WINDOW])  # Next day
        
        X = torch.stack(X)  # [time_steps, nodes, features]
        y = torch.stack(y)  # [time_steps, nodes, features]
        print(f"    X: {X.shape}, y: {y.shape}")
        
        # Normalize
        X_mean, X_std = X.mean(dim=(0, 1), keepdim=True), X.std(dim=(0, 1), keepdim=True)
        X_std[X_std == 0] = 1
        X = (X - X_mean) / X_std
        
        y_mean, y_std = y.mean(dim=(0, 1), keepdim=True), y.std(dim=(0, 1), keepdim=True)
        y_std[y_std == 0] = 1
        y = (y - y_mean) / y_std
        ...
        for epoch in range(EPOCHS):
            ...
            for X_b, y_b in tr_loader:
                ...
                # Forward: add batch dim for LSTM
                X_b = X_b.unsqueeze(0)  # [1, batch_size, nodes, features]
                
                optimizer.zero_grad()
                # Squeeze batch dim para passar para modelo (batch_size, nodes, features)
                X_b_squeezed = X_b.squeeze(0)
                pred = model(X_b_squeezed, edge_index.to(device))  # [batch_size*nodes, features]
            ...
            with torch.no_grad():
                for X_b, y_b in va_loader:
                    ...
                    pred = model(X_b_squeezed, edge_index.to(device))
                    pred = pred.view(y_b.shape[0], num_nodes, num_features)
            ...
            if va_loss < best_val:
                best_val = va_loss
                patience = 0
                torch.save(model.state_dict(), MODELS_DIR / 'model_cvli_novo_criterio.pth')
