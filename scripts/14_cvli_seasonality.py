...
def main():
    ...
    # per-bairro+city monthly totals and stats (if bairro/city present)
    if 'BairroOcor' in df_cvli.columns or 'CidadeOcor' in df_cvli.columns:
        df_cvli['bairro_up'] = df_cvli.get('BairroOcor', '').astype(str).str.upper()
        df_cvli['cidade_up'] = df_cvli.get('CidadeOcor', '').astype(str).str.upper()
        df_cvli['bairro_cidade'] = df_cvli['bairro_up'] + ' / ' + df_cvli['cidade_up']
        by_bairro_city_month = df_cvli.groupby([df_cvli['bairro_cidade'], df_cvli['month']])['is_cvli'].sum().rename('cvli_monthly').reset_index()
        by_bairro_city_month.to_csv('outputs/cvli_by_bairro_cidade_monthly.csv', index=False)

        stats = by_bairro_city_month.groupby('bairro_cidade')['cvli_monthly'].agg(['mean','std','max']).reset_index().rename(columns={'mean':'mean_monthly','std':'std_monthly','max':'peak_monthly'})
        ...
        stats.to_csv('outputs/cvli_bairro_cidade_stats.csv', index=False)
    else:
        by_bairro_city_month = None
    ...
    with md_path.open('w', encoding='utf-8') as f:
        ...
        f.write('- Método de identificação: `tipo` coluna quando presente; caso contrário filtro por `Natureza` (palavras-chave).\n')
        if tipo_present:
            f.write('- Fonte do label: coluna `tipo` usada (valores normalizados para maiúsculas).\n')
        else:
            f.write('- Fonte do label: fallback por palavras-chave em `Natureza`.\n')
        ...
        f.write(f'- Linhas originais lidas: {orig_rows}; linhas com data válida: {rows_with_date}; registros CVLI usados: {used_rows}; total CVLI: {total_cvli}.\n\n')
        ...
        if by_bairro_city_month is not None:
            f.write('Foram gerados os arquivos `outputs/cvli_by_bairro_cidade_monthly.csv` (contagem por mês, bairro/cidade) e `outputs/cvli_bairro_cidade_stats.csv` (média, desvio, CV por bairro/cidade).\n')
            # include top-5 bairros/cidades
            top5 = pd.read_csv('outputs/cvli_bairro_cidade_stats.csv').head(5)
            f.write('\nTop 5 bairros/cidades por média mensal de CVLI:\n')
            for _, r in top5.iterrows():
                f.write(f"- {r['bairro_cidade']}: média mensal={r['mean_monthly']:.2f}, pico mensal={int(r['peak_monthly'])}, CV={r['cv']:.2f}\n")
        else:
