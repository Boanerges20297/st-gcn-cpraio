...
# -*- coding: utf-8 -*-
"""
Treinador ST-GCN - Janela 90 Dias
"""
...
def main():
    ...
    try:
        # LOAD
        print("\n[1] LOADING DATA...")
        data = torch.load(DATA_DIR / 'tensors' / 'dataset_criticidade_janela90d.pt')
        dataset = data
        
        print(f"    Shape: {dataset.shape}")
        
        with open(DATA_DIR / 'tensors' / 'metadata_janela90d.json', 'r', encoding='utf-8') as f:
            meta = json.load(f)
        ...
        for epoch in range(EPOCHS):
            ...
            if va_loss < best_val:
                best_val = va_loss
                patience = 0
                torch.save(model.state_dict(), MODELS_DIR / 'model_janela90d.pth')
                torch.save({'X_mean': X_mean, 'X_std': X_std, 'y_mean': y_mean, 'y_std': y_std},
                          MODELS_DIR / 'stats_janela90d.pt')
                print(f"Ep {epoch+1:3d} | Tr: {tr_loss:.6f} | Va: {va_loss:.6f} [OK]")
            else:
                patience += 1
        ...
        with open(MODELS_DIR / 'training_history_janela90d.json', 'w') as f:
